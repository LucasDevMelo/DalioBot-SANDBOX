...
function AdicionarEstrategiaContent() {
  ...
  const PLAN_LIMITS = {
    starter: { maxRobots: 2, name: 'Starter' },
    basic: { maxRobots: 5, name: 'Basic' },
    pro: { maxRobots: Infinity, name: 'Pro' },
  };
  const userPlanName = subscription?.planName || 'starter'; // Get the plan name from the subscription or default to 'starter'
  const currentPlanLimits = PLAN_LIMITS[userPlanName as keyof typeof PLAN_LIMITS] || PLAN_LIMITS.starter;
  ...
  const handleEnviar = async (e: React.FormEvent) => {
    ...
    try {
      if (userPlanName === 'starter') {
        ...
        const userRobosRef = dbRef(db, `estrategias/${user.uid}`);
        const snapshot = await get(userRobosRef);
        if (snapshot.exists()) {
          const robos = Object.values(snapshot.val());
          if (robos.length >= currentPlanLimits.maxRobots) {
            setError(`Robot limit reached. Your ${currentPlanLimits.name} plan allows up to ${currentPlanLimits.maxRobots} robots. Please upgrade to add more.`);
            return;
          }
        }
      }
